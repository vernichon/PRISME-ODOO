...
CH_KREUZ_IMAGE_PATH = os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "static",
    "src", "img", "CH-Kreuz_7mm.png")
...
def bvr_checksum(bvr):
    table = [[0, 9, 4, 6, 8, 2, 7, 1, 3, 5], [9, 4, 6, 8, 2, 7, 1, 3, 5, 0], [
        4, 6, 8, 2, 7, 1, 3, 5, 0, 9
    ], [6, 8, 2, 7, 1, 3, 5, 0, 9, 4], [8, 2, 7, 1, 3, 5, 0, 9, 4, 6],
             [2, 7, 1, 3, 5, 0, 9, 4, 6, 8], [7, 1, 3, 5, 0, 9, 4, 6, 8, 2], [
                 1, 3, 5, 0, 9, 4, 6, 8, 2, 7
...
class AccountInvoice(models.Model):
    ...
    qrcode_qrbill = fields.Binary(
        string='QRCode', compute='_iban_qrcode', store=False)
    qrcode_status = fields.Text(
        string='QRCode Status', compute='_iban_qrcode', store=False)
            ...
            if inv.type == 'out_invoice':
                try:
                    if inv.number:
                        ...
                        inv.qrcode_status = _(
                            "The QRCode will be generated once you validate the invoice"
                        )
        ...
        with io.BytesIO() as f:
            ...
            with Image.open(f) as im:
                ...
                with logo:
                    ...
                    box = (start_x, start_y, start_x + logo.width,
                           start_y + logo.height)
    ...
    def _generate_qrbill_content(self, invoice):
        ...
        if not invoice.partner_bank_id:
            raise exceptions.except_orm(
                _('Invalid IBAN'), _('You must specify an IBAN'))
        ...
        elif not (creditor_iban.startswith("CH")
                  or creditor_iban.startswith("LI")):
            raise exceptions.except_orm(
                _('Invalid IBAN'),
                _('Only IBANs starting with CH or LI are permitted. IBAN: %s')
                % (invoice.partner_bank_id.acc_number))
        # The IBAN must be exactly 21 characters
        elif not len(creditor_iban) == 21:
            raise exceptions.except_orm(
                _('Invalid IBAN'),
                _('IBAN "%s" length (%s) must be exactly 21 characters long') %
                (invoice.partner_bank_id.acc_number, len(creditor_iban)))

        # Generate Creditor data
        creditor = self._generate_qrbill_contact_data(
            invoice.company_id.partner_id, "Creditor")
        ...
        if not currency in ["CHF", "EUR"]:
            raise exceptions.except_orm(
                _('Invalid Currency'),
                _('Currency must be either CHF or EUR (used %s)') % (currency))
        ...
        ultimate_debtor = self._generate_qrbill_contact_data(
            invoice.partner_id, "Debtor")

        # By default, no reference is provided
        ref_type = 'NON'
        ref = '\r'
        if hasattr(invoice, 'bvr_reference') and isinstance(
                invoice.bvr_reference, str):
            ...
            if tmp_ref and len(tmp_ref) > 0:
                ...
                if not len(ref) == 27:
                    raise exceptions.except_orm(
                        _('Invalid BVR Reference Number'),
                        _('BVR reference number length (%s) must be exactly 27 characters long'
                          ) % (len(ref)))
        ...
        return '\n'.join([
            qr_type, version, coding_type, creditor_iban, creditor,
            ultimate_creditor, total_amount, currency, due_date,
            ultimate_debtor, ref_type, ref
        ]).encode('latin-1')
    ...
    def _generate_qrbill_contact_data(self, contact, role):
        ...
        if not contact:
            raise exceptions.except_orm(
                _('Invalid ' + role), _(role + ' is mandatory'))

        contact_name = contact.name
        # The name must be provided and non empty
        if not contact_name or len(contact_name) == 0:
            raise exceptions.except_orm(
                _('Invalid ' + role + '\'s Name'),
                _(role + '\'s name is mandatory'))
        # The name is maximum 70 characters long
        elif len(contact_name) > 70:
            raise exceptions.except_orm(
                _('Invalid ' + role + '\'s Name'),
                _(role +
                  '\'s name "%s" length (%s) must not exceeds 70 characters') %
                (contact_name, len(contact_name)))
        ...
        # The street is maximum 70 characters long
            ...
            raise exceptions.except_orm(
                _('Invalid ' + role + '\'s Street'),
                _(role +
                  '\'s street "%s" length (%s) must not exceeds 70 characters')
                % (contact_street, len(contact_street)))
        ...
        # The house number is maximum 16 characters long
            ...
            raise exceptions.except_orm(
                _('Invalid ' + role + '\'s House Number'),
                _(role +
                  '\'s house number "%s" length (%s) must not exceeds 16 characters'
                  ) % (contact_house_number, len(contact_house_number)))

        contact_postal_code = contact.zip
        # The postal code is mandatory
        if not contact_postal_code or len(contact_postal_code) == 0:
            raise exceptions.except_orm(
                _('Invalid ' + role + '\'s Postal Code'),
                _(role + '\'s postal code is mandatory'))
        # The postal code is maximum 16 characters long
        elif len(contact_postal_code) > 16:
            raise exceptions.except_orm(
                _('Invalid ' + role + '\'s Postal Code'),
                _(role +
                  '\'s postal code "%s" length (%s) must not exceeds 16 characters'
                  ) % (contact_postal_code, len(contact_postal_code)))

        contact_city = contact.city
        # The city is mandatory
        if not contact_city or len(contact_city) == 0:
            raise exceptions.except_orm(
                _('Invalid ' + role + '\'s City'),
                _(role + '\'s city is mandatory'))
        # The city is maximum 35 characters long
        elif len(contact_city) > 35:
            raise exceptions.except_orm(
                _('Invalid ' + role + '\'s City'),
                _(role +
                  '\'s city "%s" length (%s) must not exceeds 35 characters') %
                (contact_city, len(contact_city)))

        contact_country = contact.country_id.code
        # The country code is mandatory
        if not contact_country or len(contact_country) == 0:
            raise exceptions.except_orm(
                _('Invalid ' + role + '\'s Country Code'),
                _(role + '\'s country code is mandatory'))
        # The standard uses ISO 3166-1, so the country code must be 2 characters long
        if not len(contact_country) == 2:
            raise exceptions.except_orm(
                _('Invalid ' + role + '\'s Country Code'),
                _(role +
                  '\'s country code "%s" length (%s) must be exactly 2 characters long (according to ISO 3166-1)'
                  ) % (contact_country, len(contact_country)))

        # Each fields are separated by a line break
        return '\n'.join([
            contact_name, contact_street, contact_house_number,
